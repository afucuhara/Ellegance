      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="p-6 bg-[#f7f3ec] rounded-2xl border border-[#dce5e7] space-y-3 mt-6">
              <div className="flex items-center gap-3 text-[#07516f] font-bold">
                <HelpCircle className="w-5 h-5 text-[#0787c8]" />
          ...
          <div className="lg:col-span-7 border-t border-[#dce5e7]">
            {FAQS.map((faq) => {
              const isOpen = openId === faq.id;
              return (
                <div
                  key={faq.id}
                  className="border-b border-[#dce5e7] transition-colors"
                >
                  <button
                    onClick={() => toggleFaq(faq.id)}
                    className="w-full py-6 flex items-center justify-between text-left gap-4 hover:text-[#0787c8] transition-colors"
                  >
                    <span className="text-lg sm:text-xl font-bold text-[#132a34] leading-snug">
                      {faq.question}
                    </span>
                    <span className="p-2 rounded-full bg-[#f7f3ec] text-[#0787c8] shrink-0">
